...
const PlayerSetup = ({ onStartGame }) => {
  ...
  const startGameWithSetup = () => {
    ...
    onStartGame(shuffledPlayers, cardSize);
  };

  return (
    <CCard>
      <CCardHeader>
        <h4>Gold Digger Game Setup</h4>
      </CCardHeader>
      <CCardBody>
        <CForm>
          <div className="mb-3">
            ...
            <CFormInput
              ...
              value={playerCount}
              onChange={(e) => {
                const value = Math.min(100, Math.max(2, parseInt(e.target.value) || 2));
          ...
          <div className="mb-3">
            <CFormLabel htmlFor="cardSize">Card Size ({playerCount}-100)</CFormLabel>
            <CFormInput
              id="cardSize"
              type="number"
              min={playerCount}
              ...
              value={cardSize}
              onChange={(e) => {
                const value = Math.min(100, Math.max(playerCount, parseInt(e.target.value) || playerCount));
                setCardSize(value);
              }}
            />
          </div>
...
const BingoCard = ({ card, color, isCurrentPlayer, onCellClick, playerName }) => {
  return (
    <motion.div
...
const GoldDiggerMulGame = () => {
  ...
  const handleCellClick = (playerIndex, cellIndex) => {
    if (gameState !== 'playing' || playerIndex !== currentPlayerIndex) return;
...
export default GoldDiggerMulGame;